...
class YOLO_Human:
    ...
    def postprocess(self, frame, outs):
        ...
        for i, box in enumerate(selected_boxes):
            ...
            # print('center: {}'.format(centerCount_list))
            #P1 = (int(centerList[0][0]), int(centerList[0][1]))  # 0
            #P2 = (int(centerList[1][0]), int(centerList[1][1]))  # 1
            ...
            if len(centerCount_list) > 1:
                for C, center_First in enumerate(centerCount_list):
                    ...
                    #cv2.line(self.out_frame, P1, P2, (0, 255, 255), 1)
                    #cv2.circle(self.out_frame, center, 3, (0, 255, 255), -1)
                    #cv2.rectangle(self.out_frame, (left, top), (right, bottom), (255, 178, 50), 3)
            # ----------------- Center ---------------- #
        ...
        return len(selected_boxes)  # --count--#

    def drawPred(self, classId, conf, left, top, right, bottom, subtract):
        #right = left + width
        #bottom = top + height
        # Draw a bounding box. #
        # cv.rectangle(out_frame, (left, top), (right, bottom), (255, 178, 50), 3)
        # label = '%.2f' % conf
        # Get the label for the class name and its confidence #
        # if classes[classId]:
        if self.classes[classId] == 'person':
            #if subtract:
                #boundRect = cv2.rectangle(self.out_frame, (left, top), (right, bottom), (0, 0, 255), 3)
            #else:
               # boundRect = cv2.rectangle(self.out_frame, (left, top), (right, bottom), (255, 178, 50), 3)

            # ----------------- Center ---------------- #
            #center = (int((left + right) / 2), int((top + bottom) / 2))
            #cv2.circle(self.out_frame, center, 3, (0, 255, 0), -1)
            #cv2.circle(self.out_frame, (int(left + width/2), int(top + height/2)), 3, (0, 255, 0), -1)
            # ----------------- Center ---------------- #

            Percent = '%.2f' % conf
            assert (classId < len(self.classes))
            label = '%s:%s' % (self.classes[classId], Percent)
            cv2.putText(self.out_frame, str(Percent), (left, top), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 1)
    ...
    def mouse_drawing(self, event, x, y, flags, params):
        # global DownPoint1, DownPoint2, DownPoint3, DownPoint4, Click, Mouse_count
        # global DownPoint, CLick
        # ----------Mouse 1-------
        ...
        while cv2.waitKey(1) < 0:
            ...
            if self.Mouse_count:  # True
                ...
                if hasFrame:
                    ...
                    selected_boxes = self.postprocess(frame, outs)
